...
def create_argparser():
    parser = ArgumentParser(
        description="Scrooge – A command line based manager for personal finances"
    )
    ...
    track_parser.add_argument(
        "category", choices=["fixed", "savings", "investments", "free"]
    )
    track_parser.add_argument("tags", nargs="*")
    track_parser.add_argument(
        "--description", "-d", help="An optional description for the expense"
    )

    summary_parser = subparsers.add_parser(
        "show", help="Print the summary of expenses for a specific time frame"
    )
    ...
    monthly_parser = subparsers.add_parser(
        "month", help="Print info about the given month"
    )
    ...
    backup_parser.add_argument(
        "action", nargs="?", choices=["create", "restore"], help="create is default"
    )
    backup_parser.set_defaults(func=commands.backup_data)

    import_parser = subparsers.add_parser(
        "import", help="Import and verify multiple expenses from a .csv file"
    )
    import_parser.add_argument(
        "file",
        help="The file to import from. If the given file does not exist, this command may create a template file",
    )
