...
def amount_row(table, title, amount, *mod):
    table.add_row(with_modifiers(title, *mod),
                  fmt_amount(amount, *mod))
...
class DescriptionGroup:
    ...
    def insert_into_table(self, table):
        table.add_row(f'[bold]{self.title}[/bold]',
                      self.title_fmt_amount(self.total, "bold"))
...
def print_expenses_grouped_by_tags(expenses):
    ...
    table = Table("descriptor", Column(header="Amount", justify="right"),
                  box=None, 
                  show_header=False)
    for tags, expenses in groups.items():
        ...
        table.add_row()
    table.add_row(with_modifiers("Total", "bold"),
                  fmt_amount(all_expenses))
    print(table)
...
def analyse_monthly_log(log: MonthlyLog, important_tags):
    ...
    unaccounted = log.available - sum(map(lambda e: e.amount, log.expenses)) \
        - log.allocated_for_savings - log.allocated_for_investments
    ...
    category_into_table("Amount invested", SpendingCategory.investments, 
                        log.allocated_for_investments)
    category_into_table("Amount saved", SpendingCategory.savings,
    ...
    for tag in important_tags:
        ...
        if len(matching_expenses) > 0:
            ...
            table.add_row(tag.name, 
                          fmt_with_percentage(amount, amount / log.available))
    print(table)
